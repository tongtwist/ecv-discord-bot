...
export const setup: TCommand = {
	...
	data: new SlashCommandBuilder()
		...
		.setDescription("Retrieve and eventually store the OpenAI API key to use with a Discord user (yourself by default)")
		.addStringOption(
	...
	execute: async (interaction: ChatInputCommandInteraction): Promise<void> => {
		...
		const userDesignation: [string, string] = user.id === interaction.user.id
			? ["You", "r"]
			: [`${user.username}`, "'s"]
